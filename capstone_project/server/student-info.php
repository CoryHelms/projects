...
$getgogID = "SELECT googleID FROM Student WHERE studentID = '$userID'";
$gogID = mysqli_query($conn, $getgogID);
if (mysqli_num_rows($gogID) > 0) {
    $row = mysqli_fetch_array($gogID);
    $gogID = $row[0];
    $_SESSION['studentGoogleID'] = $gogID;
}

//If form is already completed, redirect
$query1 = "SELECT IFNULL(zip, 0) FROM Student WHERE studentID = ?";
...
if(isset($_POST["Save"])) {
    ...
    $userPreferred = mysqli_real_escape_string($conn, $_POST["preferred"]);
    ...
    function emptyInputSignup($userStreet, $userCity, $userState, $userZip, $userBio, $userGender) {
        $answer;
        if (empty($userCity) || empty($userState) || empty($userZip) || empty($userBio) || empty($userGender)) {
            $answer = true;
        } else {
            $answer = false;
        }
        ...
        if (emptyInputSignup($userStreet, $userCity, $userState, $userZip, $userBio, $userGender) !== false) {
            header("location: student-info.php?error=emptyinput");
            exit();
        }

    //Add user info to Student table in database after validation
    if (empty($userPreferred) == FALSE) {
        $sql = "UPDATE Student SET preferredName = '$userPreferred' WHERE studentID = '$userID'";
        $query = mysqli_query($conn, $sql);
    }

    if (empty($userAddType) == FALSE) {
        $sql = "UPDATE Student SET addType = '$userAddType' WHERE studentID = '$userID'";
        $query = mysqli_query($conn, $sql);
    }

    $sql = "UPDATE Student SET str = '$userStreet', city = '$userCity', state = '$userState', zip = '$userZip', bio = '$userBio', gender = '$userGender'
    ...
    <body>
        ...
        <main>
            <div class="additional_container">
                <h2 class="additional_h2"> Additional Student Information</h2><br>
                
                <form action="student-info.php" method="post">
                    <label for="preferred">Preferred Name:</label><br>
                    <input type="text" id="add_preferred" name="preferred" size="40" placeholder="Cool Name (optional)"><br>
                    
                    <label for="street">Street Address:</label><br>
                    <input type="text" id="street" name="street" size="40" placeholder="123 Cool St (required)" required><br>

                    <label for="addType">APT / STE / Other:</label><br>
                    <input type="text" id="addType" name="addType" size="40" placeholder="APT 0 (optional)"><br>
                    
                    <label for="city">City:</label><br>
                    <input type="text" id="city" name="city" size="40" placeholder="Cool City (required)" required><br>
                    ...
                    <textarea class="textarea" id="bio" name="bio" placeholder="Tell us about yourself! (required)" required></textarea> <br>
                    
                    <label for="gender">Preferred Gender:</label><br>
                    <input type="text" id="add_gender" name="gender" placeholder="Cool Gender (required)" required><br>
                    
                    <?php
                    //Display error
                    if (isset($_GET["error"])) {
                        if ($_GET["error"] == "emptyinput") {
                            echo "<p>Please fill in required fields</p>";
                            }
                        }
                    ?>
